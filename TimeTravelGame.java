...
public class TimeTravelGame {
    ...
    //     }
    //     return false;
        String[] zombieSurvivalPack = new String[8];

        Scanner scanner = new Scanner(System.in);
        System.out.println("Welcome to the Zombie Survival Pack Builder!");
        System.out.println("You can choose 8 items to include in your survival pack.\n");

        for (int i = 0; i < zombieSurvivalPack.length; i++) {
            System.out.print("Enter item " + (i + 1) + ": ");
            String item = scanner.nextLine();
            zombieSurvivalPack[i] = item;
        }
        
        for (int j = 0; j < zombieSurvivalPack.length; j++) {
            System.out.println("Item " + (j + 1) + ": " + zombieSurvivalPack[j]);
        }
